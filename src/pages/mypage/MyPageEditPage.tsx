...
export const MyPageEditPage = ({
  ...
  const addLocation = useCallback((loc: UserAddress) => {
    setLocations(prev => {
      if (prev.length >= 5) {
        alert("위치는 최대 5개까지 추가 가능");
        return prev;
      }
      if (prev.some(l => l.buildingName === loc.buildingName && l.streetAddr === loc.streetAddr))
        return prev;
      return [...prev, loc];
    });
  }, []);
  ...
  const onCompleteClick = async () => {
    ...
    try {
      ...
      if (profileImageKey && profileImageKey.trim() !== "") {
        payload.imgKey = profileImageKey;
      }
    ...
    <>
      ...
      <div className="flex flex-col">
        ...
        <div className="flex justify-center mb-8 relative">
            <img
              src={profileImage || BaseProfileImg}
              alt="profile"
        ...
        <div className="mb-8 flex flex-col items-start">
          <div className="w-full text-left flex flex-col gap-2">
            <div className="flex px-1 gap-[2px] items-center">
              <p className="header-h5">생년월일</p><CicleSRED />
            </div>
            <input
              type="text"
              className="w-full rounded-xl border-gy-200 border py-[0.625rem] px-3 focus:outline-none focus:border-active"
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-4">
              ...
              {dropdownOpen && !disabled && (
                <ul className="absolute mt-1 z-10 w-40 border rounded-xl border-gy-200 bg-white shadow max-h-36 overflow-y-auto">
                  {levelOptions.map((item, idx) => (
                    <li
                      key={idx}
                      onClick={() => { setSelectedLevel(item); setDropdownOpen(false); }}
            ...
            <button
              type="button"
              onClick={() => { const n = !disabled; setDisabled(n); if (n) { setDropdownOpen(false); setSelectedLevel(""); } }}
              className="flex items-center gap-1"
            >
              {disabled ? <CheckCircledFilled className="w-4 h-4 text-[#FF4D4F]" /> : <CheckCircled className="w-4 h-4 text-gray-400" />}
              급수 없음
            </button>
          </div>
        </div>
        ...
        <div className="mt-8">
          <div className="flex justify-between items-center mb-1">
            ...
            <button className="rounded-lg bg-[#F4F5F6] body-rg-500 px-4 py-2" onClick={toggleEditMode}>
              {editMode ? "저장" : "수정"}
            </button>
          </div>
          <div className="flex flex-col gap-2 text-start">
            {locations.sort((a, b) => a.addrId === selectedId ? -1 : b.addrId === selectedId ? 1 : 0)
              .map((loc, index) => (
              <Location
                key={loc.addrId}
                className="w-full"
