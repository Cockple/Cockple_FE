...
import { getMyProfile, patchMyProfile, getMyProfileLocations, deleteAddress, setMainAddress } from "../../api/member/my";
...
export const MyPageEditPage = ({
  ...
  const [locations, setLocations] = useState<UserAddress[]>(initialLocationsProp);
  ...
  const levelOptions = ["왕초심","초심","D조","C조","B조","A조","준자강","자강"];
  const keywordLines = [["브랜드 스폰", "가입비 무료"], ["친목", "운영진이 게임을 짜드려요"]];
  ...
  const isDataChanged = useCallback(() => {
    ...
    const currentIds = locations.map(l => l.addrId).sort().join(",");
    const initialIds = initialData.locations.map(l => l.addrId).sort().join(",");
    return currentIds !== initialIds;
  }, [name, selectedLevel, disabled, profileImage, locations]);

  const addLocation = (loc: UserAddress) => {
    setLocations(prev => {
      if (prev.length >= 5) { alert("위치는 최대 5개까지 추가 가능"); return prev; }
      if (prev.some(l => l.buildingName === loc.buildingName && l.streetAddr === loc.streetAddr)) return prev;
      return [...prev, loc];
    });
  };
  ...
  useEffect(() => {
    ...
    const updateMain = async () => {
      ...
      } catch (err) { alert("대표 주소 변경 실패"); console.error(err); }
    };
    updateMain();
  }, [selectedId]);
  ...
  const onBackClick = () => isDataChanged() ? setIsModalOpen(true) : navigate("/myPage");
  ...
  const onCompleteClick = async () => {
    if (!name.trim()) { alert("이름은 반드시 입력해야 합니다."); return; }
    if (!isDataChanged()) { navigate("/myPage"); return; }
    ...
    try {
      ...
      const payload = { memberName: name, birth: formattedBirth, level: mappedLevel, keywords: mappedKeywords, imgKey: profileImageKey || "" };
      await patchMyProfile(payload);
      alert("프로필이 성공적으로 수정되었습니다.");
      navigate("/myPage");
    } catch (err) { console.error(err); alert("프로필 수정 실패"); }
  };
    ...
    <>
      ...
      <div className="flex flex-col">
        ...
        <div className="flex justify-center mb-8 relative">
          <img src={profileImage || "프로필"} alt="profile" className="w-24 h-24 rounded-full object-cover" onClick={onProfileClick} />
          <div className="absolute bottom-0 right-[calc(50%-40px)] bg-white shadow-ds100 rounded-full p-1"><Camer_gy_400 className="w-4 h-4" /></div>
          <input type="file" accept="image/*" ref={fileInputRef} className="hidden" onChange={onFileChange} />
        </div>

        {/* 이름 */}
        <div className="mb-8">
          <label className="flex items-center text-left header-h5 mb-1">이름<CicleSRED /></label>
          <div className="relative">
            <input type="text" value={name} onChange={handleNameChange} className="w-full rounded-xl border-gy-200 border py-[0.625rem] px-3 focus:outline-none focus:border-active " />
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[#C0C4CD] body-rg-500">({name?.length ?? 0}/17)</span>
          </div>
        </div>
        ...
        <div className="mb-8 flex justify-between items-center">
          ...
          <div className="flex items-center header-h5 gap-2">
            {gender === "female" ? <>여성 <Female className="w-4 h-4"/></> : <>남성 <Male className="w-4 h-4"/></>}
          </div>
        </div>
        ...
        <div className="mb-8 flex flex-col items-start">
          <div className="w-full">
            <div className="text-left flex flex-col gap-2">
              <div className="flex px-1 gap-[2px] items-center"><p className="header-h5">생년월일</p><CicleSRED /></div>
              <input type="text" className="w-full rounded-xl border-gy-200 border py-[0.625rem] px-3 focus:outline-none focus:border-active " value={selectedDate} onClick={() => setOpenModal(true)} />
              {openModal && (
                <div id="date-picker-overlay" className="fixed inset-0 bg-black/50 z-40 flex items-center justify-center" onClick={e => (e.target as HTMLElement).id === "date-picker-overlay" && handleCloseOverlay()}>
        ...
        {/* 급수 선택 */}
        <div className="mb-8">
          <label className="flex items-center text-left header-h5 mb-1">전국 급수<CicleSRED /></label>
          <div className="flex items-center gap-4">
            <div className="relative w-40">
              <button className="border px-3 py-[0.625rem] flex justify-between gap-2 rounded-xl border-gy-200 w-40 h-11 cursor-pointer" onClick={() => !disabled && setDropdownOpen(!dropdownOpen)}>
                <span className={disabled ? "text-gy-500":"text-black"}>{selectedLevel}</span>
              ...
              {dropdownOpen && !disabled && (
                <ul className="absolute mt-1 z-10 w-40 border rounded-xl border-gy-200 bg-white shadow max-h-36 overflow-y-auto">
                  {levelOptions.map((item, idx) => <li key={idx} onClick={() => {setSelectedLevel(item); setDropdownOpen(false);}} className="text-start cursor-pointer w-full px-3 py-[0.625rem] hover:bg-gy-100 rounded-xl">{item}</li>)}
                </ul>
              )}
            </div>
            <button type="button" onClick={() => {const n = !disabled; setDisabled(n); if(n) {setDropdownOpen(false); setSelectedLevel("");}}} className="flex items-center gap-1">
              {disabled ? <CheckCircledFilled className="w-4 h-4 text-[#FF4D4F]" /> : <CheckCircled className="w-4 h-4 text-gray-400" />}급수 없음
            </button>
          </div>
        </div>
        ...
        <div className="mt-8">
          <div className="flex justify-between items-center mb-1">
            ...
            <button className="rounded-lg bg-[#F4F5F6] body-rg-500 px-4 py-2" onClick={toggleEditMode}>{editMode ? "저장":"수정"}</button>
          </div>
          <div className="flex flex-col gap-2 text-start">
            {locations.sort((a,b)=> a.addrId===selectedId?-1:b.addrId===selectedId?1:0).map((loc,index)=>(
              <Location key={loc.addrId} className="w-full" isMainAddr={loc.buildingName} streetAddr={loc.streetAddr} initialClicked={loc.addrId===selectedId} disabled={editMode && index===0} editMode={editMode} onClick={()=>!editMode && setSelectedId(loc.addrId)} onDelete={()=>handleDelete(loc.addrId)}/>
            ))}
          </div>
        </div>
            ...
            {keywordLines.map((line,i)=>(
              <div key={i} className="flex gap-4 flex-wrap">
                {line.map(k=>{
                  ...
                  return <TextBox key={k} isSelected={isSelected} className={`py-2 rounded-xl whitespace-nowrap w-auto max-w-full ${k==="친목"?"px-[1.4rem]":"px-[2.7rem]"}`} onClick={()=>setSelectedKeywords(prev=>isSelected?prev.filter(x=>x!==k):[...prev,k])}>{k}</TextBox>
                })}
              </div>
            ))}
          </div>
        ...
        <div className="mt-8 mb-8 flex justify-center">
          <Btn_Static kind="GR400" size="L" label="수정 완료" shadow="shadow-ds100" onClick={onCompleteClick}/>
        </div>
      </div>
    </>
  );
