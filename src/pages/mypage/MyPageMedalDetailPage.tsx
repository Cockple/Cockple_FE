    ...
    const fetchData = async () => {
      try {
        ...
        setMedalDetail({
          ...
          medalType: (data as any).medalType ?? "NONE", 
        });
      } catch (err) {
        console.error(err);
        setMedalDetail(null);
    ...
    <div className="w-full max-w-[444px] mx-auto min-h-screen bg-white relative overflow-x-hidden">
      ...
      <div className="flex justify-between items-center mt-5">
        ...
        <p className="body-md-500">
          {medalDetail.type && medalDetail.level
            ? `${participationMap[medalDetail.type] ?? medalDetail.type} - ${levelMap[medalDetail.level] ?? medalDetail.level}`
            : "-"}
        </p>
      </div>
