...
export const GameFinishedTab = ({ exerciseId }: GameFinishedTabProps) => {
  ...
  return (
    <div className="flex flex-col items-center mt-2 w-full bg-[#F5F6F8] p-4 rounded-xl">
      {/* 01 코트 / 좌우 화살표 */}
      <div className="flex items-center gap-6 mb-6">
        ...
        <div className="px-5 py-2 bg-gray-200 text-gray-700 rounded-lg body-md-700">
          01 코트
        </div>
      ...
      <div className="flex flex-col gap-4 w-full">
        {mockMatches.map(match => (
          <div key={match.id} className="bg-white rounded-2xl shadow-ds300 p-4 w-full flex flex-col gap-3">
              ...
              <div className="grid grid-cols-2 gap-2 w-full">
                {match.teamA.map((p, idx) => (
                  <div key={idx} className="bg-[#FFEBEF] rounded-lg py-[0.375rem] flex items-center justify-center gap-1">
                    ...
                    <span className="text-[#767B89] body-rg-500">{p.tag}</span>
                  </div>
                ))}
              </div>
              <div className="grid grid-cols-2 gap-2 w-full">
